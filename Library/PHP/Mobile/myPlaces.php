	...
	if ($conn->connect_error) {
		...
		if ($pick->num_rows > 0) {
			while ($row = $pick->fetch_assoc()) {
				...
				$placeId = $row['PLACEID'];
				$build = "
					<button type='button' onclick='previewPlace(\"$placeIdNum\")' class='placeName'>
						$placeId
					</button>
				";
...
echo "	
				...
				<div id='mapContainer'>
					...
					<button class='showMenu' onclick='showHideMapMenuM();'>&#xf0c9;</button>
					<div id='menu-container' onclick='showHideMapMenuM();'>
						<div id='menu' onclick='event.cancelBubble=true;if(event.stopPropagation) event.stopPropagation();return false;'>
							<button onclick='showTagContainer()'>Tag this place</button>
							<button onclick='hideMapM()'>Hide map</button>
						</div>
					</div>

					<div id='tag-container' onclick='hideTagContainer();'>
						<div id='taggedFriends' onclick='event.cancelBubble=true;if(event.stopPropagation) event.stopPropagation();return false;'>
						</div>
