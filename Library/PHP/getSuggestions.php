<?php
	...
	if ( file_exists("../Authors/$sender/Following.txt") ) {
		$following_list = fopen( "../Authors/$sender/Following.txt", "r" ) or die( "Fatal: Could not start opening." );
	...
	if ( !empty( $suggestions_ ) ) {
		foreach ( $suggestions_ as $author_uid ) {
			if ( !empty( $author_uid ) ) {
				$lineCount = 0;
				if ( $author_uid != NULL) {
					$user_meta = fopen( "../Authors/$author_uid/config.txt", "r" ) or die( "Fatal: Could not load." );
					...
					//Build and print
					$build .= "
						<button onclick=\"window.location='openBloger.php?$author_uid'\">
							<div style='background: url(\"$profile_picture\"); background-size: cover; background-position: 50%;' class='img'></div>
		...
		$build = "<h2>There are no new suggestion for you :-(</h2>";
	}
