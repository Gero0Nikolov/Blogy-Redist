<?php
	...
	//Check if the visitor is currently a follower
	$isFollower = 0;
	...
	<html>
		<head>
			...
			<script type='text/javascript'>
				...
				function callBack() {
					loops = 1;
					loadStories(lastId, \"1\", 0, \"".$blogerSender."\");
				}

				var flag = 0;
			</script>
		</head>
		<body onload='loadStories(-1, \"1\", 0, \"".$blogerSender."\"); loadBadges(0, 1);' onscroll='checkPos()'>
";
...
";
	...
	if (!in_array($blogerSender, $blockedPersons)) {
		$blockUnblock = "<button type='button' class='split' onclick='blockUser(\"$blogerSender\")'>Block user</button></br>";
	} else {
		$blockUnblock = "<button type='button' class='split' onclick='unBlockUser(\"$blogerSender\")'>Unblock user</button></br>";
	}
	
echo "
						$button
						$blockUnblock
						<button type='button' onclick='buildReportContainer(0, \"$blogerSender\");'>Report user</button>
					</div>
				</div>
";
